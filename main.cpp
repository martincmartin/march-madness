...
// Consider a pure Monte Carlo for the pre-round-of-64 optimizer.  Could use
// sequential analysis to stop early:
// https://stats.stackexchange.com/questions/124825/sequential-testing-for-a-bernoulli-variable
// Basically, when we find a new best, run 10,000,000 Monte Carlo iters to
// estimate it's probability accurately.  Given a new bracket, set p0 =
// best_prob - 0.001 and p1 = best_prob + 0.001.  This lets you end when you've
// determined that it's < p0 or > p1.  Set alpha = beta = 2^-15 / 1000 or
// something, the run times seem relatively insensitive to the values of alpha
// and beta.  See Walds, 1945 section 5.2 for more details.
//
// MaxSPRT might work even better:
// https://www.tandfonline.com/doi/full/10.1080/07474946.2011.539924
// However, it looks a lot harder to implement.
//
// Also, instead of looking at all possible combinations from Sweet 16 on, you
// could look at all combinations of 1, 2 & 3 flips.  That should take a
// comparable amout of time, 41,727 vs 32,768, and also cover rounds of 64 and
// 32.  Another insight is that you should only look at combos where one flip is
// downstream of another flip.  So when you flip something in round of 64, look
// at all 2^5 = 32 combos of games directly affected in later rounds.
...
constexpr const char *PROBS_FNAME =
    YEAR "/{}-fivethirtyeight_ncaa_forecasts.csv";
...
// could do in 1.6 seconds.  So we want the threshold higher than that.  Update:
// No we don't, the extra precision doesn't make a difference, and it speeds up
// a LOT by lowering it.
constexpr size_t MONTE_CARLO_THRESHOLD = 1'000'000;
// 100,000,000 iters runs out of RAM on Mac Airbook (8 GB).
// 100,000 should be enough, even for all_optimzie().
// Emperically, 1,000 makes a wrong choice.  So does 10,000, although (a) it's a
// single wrong choice, so could be caught by looking at single flips, and (b)
// it makes such a small difference, < 0.1%, that it's really in the noise.
// Less than one extra win every 1,000 years. Game 50, UCLA vs Baylor in 2022.
constexpr size_t MONTE_CARLO_ITERS = 100'000;
...
array<uint64_t, NUM_BRACKETS> entries{
    ...
    58407997,  // Dan (Dan Murphy, dmurph888).  2MW Auburn to win, 3E Purdue
               // over 2E Kentucky for Final 4.  2S Villanova over 1S Arizona.
    58468455,  // Eileen-er-iffic (Eileen (Dan's Sister), The_MRF_NYC) 3E Purdue
               // to win.  4MW Providence over 1 MW Kansas.
    61439241,  // Vakidis (Billy (Maureen's Husband), wgarbarini) 3E Purdue over
               // 2E Kentucky. 2S Villanova over 1S Arizona.  5MW Iowa over 1MW
               // Kansas.  2S Villanova over 1MW Kansas to make it to
               // championship.
    62484411,  // Owe'n Charlie '22 (Uncle Dennis, tiger72pu).  2S Villanova
               // over 1S Arizona.
    65481077,  // Maureen's Annual Bonus (Joe (Eileen's Husband),
               // gettinpiggywitit) 1S Arizona to win.  Sweet 16: 3S Tennessee
               // over 2S Villanova.  3MW Wisconsin over 2MW Auburn.
    69629125,  // RPcatsmounts! espn88461517 (Ryan Price, Dan's step brother).
               // 1MW Kansas to win. 3E Purdue over 2E Kentucky. 3S Tennessee
               // over 1S Arizona. 61783453,  # Villa-Mo-va 1 (Maureen (Dan's
               // Sister), Villa-Mo-va)
};
...
class Rand {
   public:
    ...
    double uniform() {
        return (1.0 / static_cast<double>(numeric_limits<uint64_t>::max())) *
               uint64();
    }
...
string get_forecasts() {
    return get_with_caching(
        "https://projects.fivethirtyeight.com/march-madness-api/2022/"
...
void parse_probs() {
    ...
    for (auto &row : csv.rows) {
        ...
        if (iter == eid_to_team.end()) {
            ...
            if (row[playin] == "0") {
                cout << "Team " << row[name] << " eid " << row[id]
                     << " from probability CSV not recognized.\n";
                abort();
            }
            continue;
        }
...
double game_prob(team_t first, team_t second, team_t winner, int round) {
    ...
    double result = get_prob(winner, round) /
                    (get_prob(first, round) + get_prob(second, round));
    if (isnan(result)) {
        cout << "first: " << (int)first << " " << teams[first].name
             << ", second: " << (int)second << " " << teams[second].name
             << ", winner: " << (int)winner << ", round: " << round << endl;
...
string URL_FORMAT =
    "https://fantasy.espn.com/tournament-challenge-bracket/" YEAR
    "/en/entry?entryID={}";
...
const array<const string, NUM_ROUNDS> round_names{
    "Champ", "Final4", "Elite8", "Sweet16", "Roundof32", "Roundof64",
};

unordered_map<game_t, string> INDEX_TO_NAME{
    {0, "Round of 64"},           {32, "Round of 32"},
    {32 + 16, "Sweet 16"},        {32 + 16 + 8, "Elite 8"},
    {32 + 16 + 8 + 4, "Final 4"}, {32 + 16 + 8 + 4 + 2, "Championship"},
};
...
string to_string(const Matchup &matchup) {
    return to_string(matchup.id) + ": " + teams[matchup.first_team].name +
           ", " + teams[matchup.second_team].name + ", " +
...
string to_string(const Bracket &bracket) {
    ...
    for (game_t match = 0; match < NUM_GAMES; ++match) {
        ...
        result +=
            to_string(match) + ": " + teams[bracket.picks[match]].name + "\n";
    }
    return result;
}
...
Bracket make_bracket(const array<bool, NUM_GAMES> &who_wins,
    ...
    for (game_t match = 0; match < NUM_GAMES; ++match) {
        if (match < 32) {
            ...
            bracket.picks.push_back(
                bracket.picks[input(match) + (who_wins[match] ? 0 : 1)]);
        }
    }
...
pair<Bracket, array<bool, NUM_GAMES>> make_most_likely_bracket() {
    ...
    for (game_t match = 0; match < NUM_GAMES; ++match) {
        ...
        // fixed it up.
        double first_prob =
            game_prob(first_team, second_team, first_team, round);
...
void compare(const Bracket &bracket1, const Bracket &bracket2) {
    ...
    for (game_t match = 0; match < NUM_GAMES; ++match) {
        ...
        cout << fmt::format(
            "{} {:<2} {:<20} {}\n",
            bracket1.picks[match] == bracket2.picks[match] ? " " : "*", match,
...
struct Var : public BoolExpr {
    ...
    Var(string name, size_t index, bool first)
        : name(name), index(index), first(first) {}
...
class InfixExpr : public BoolExpr {
   public:
    ...
    InfixExpr(unordered_set<shared_ptr<BoolExpr>> kids)
...
class OrExpr : public InfixExpr {
   public:
    OrExpr(unordered_set<shared_ptr<BoolExpr>> kids)
        : InfixExpr{std::move(kids)} {}
...
class AndExpr : public InfixExpr {
   public:
    AndExpr(unordered_set<shared_ptr<BoolExpr>> kids)
        : InfixExpr{std::move(kids)} {}
...
template <typename ExprT>
unordered_set<shared_ptr<BoolExpr>> flatten(
    ...
    for (const auto &child : children) {
        if (auto target_expr = dynamic_cast<ExprT *>(child.get())) {
            new_kids.insert(target_expr->children.begin(),
                            target_expr->children.end());
        } else {
...
pair<shared_ptr<BoolExpr>, bool> factor(const shared_ptr<BoolExpr> &first_in,
                                        const shared_ptr<BoolExpr> &second_in);
...
template <typename OuterExprT>
shared_ptr<BoolExpr> special_helper(
    const unordered_set<shared_ptr<BoolExpr>> &source_children) {
    ...
    for (const auto &source_child : source_children) {
        if (auto source_child_var = dynamic_cast<Var *>(source_child.get())) {
            ...
            for (auto dual_iter = source_children.begin();
                 dual_iter != source_children.end(); ++dual_iter) {
                if (auto dual = dynamic_cast<InnerExprT *>(dual_iter->get())) {
                    ...
                    if (negation_iter != dual->children.end()) {
                        ...
                        //
                        // At this point:
                        // source_child, source_child_var point to the "outer"
                        // var, child of source. dual_iter, dual point to the
                        // dual, child of source. negation_iter points to the
                        // negation of var, child of dual. cout << "Found " <<
                        // source_child_var->to_string() << " in outer, and " <<
                        // to_string(*negation_iter) << " in inner.\n";

                        bool raised_node = false;
                        unordered_set<shared_ptr<BoolExpr>> result;
                        for (auto dual_iter2 = source_children.begin();
                             dual_iter2 != source_children.end();
                             ++dual_iter2) {
                            if (dual_iter2 != dual_iter) {
                                result.insert(*dual_iter2);
                            } else {
                                // For this child, take everything but the
                                // negation var.
                                unordered_set<shared_ptr<BoolExpr>>
                                    new_dual_children;
                                for (auto dual_child_iter =
                                         dual->children.begin();
                                     dual_child_iter != dual->children.end();
                                     ++dual_child_iter) {
                                    if (dual_child_iter != negation_iter) {
                                        new_dual_children.insert(
                                            *dual_child_iter);
                                    }
                                }
                                ...
                                if (new_dual_children.size() == 1) {
                                    ...
                                    result.insert(make_shared<InnerExprT>(
                                        std::move(new_dual_children)));
                                }
                            }
                        }
                        // cout << "Replacing " <<
                        // to_string(make_shared<OuterExprT>(source_children))
                        // << " with " <<
                        // to_string(make_shared<OuterExprT>(result)) << "\n";
                        if (raised_node) {
    ...
    for (const auto &child : children) {
        if (auto child_var = dynamic_cast<Var *>(child.get())) {
            if (children.find(child_var->negation()) != children.end()) {
                return make_shared<typename Dual<ExprT>::type>(
                    unordered_set<shared_ptr<BoolExpr>>{});
            }
        }
    }
    ...
    if (try_factor) {
        ...
        while (kids.size() >= 2) {
            ...
            for (size_t j = 0; j < kids.size() - 1; ++j) {
                auto [expr, factored] =
                    factor<ExprT>(kids[j], kids[kids.size() - 1]);
                if (factored) {
                    modified = true;
                    got_one = true;
...
pair<shared_ptr<BoolExpr>, bool> factor(const shared_ptr<BoolExpr> &first_in,
    ...
    const InnerExprT *second =
        dynamic_cast<const InnerExprT *>(second_in.get());

    if (!first || !second) {
        return make_pair(
            make_shared<OuterExprT>(
                unordered_set<shared_ptr<BoolExpr>>{first_in, second_in}),
            false);
    }
    ...
    if (factored.common.empty()) {
        return make_pair(
            make_helper<OuterExprT>(
                unordered_set<shared_ptr<BoolExpr>>{first_in, second_in}),
            false);
    }

    if (factored.reduced_first.empty() || factored.reduced_second.empty()) {
        return make_pair(make_helper<InnerExprT>(std::move(factored.common)),
                         true);
    }

    auto temp = make_helper<OuterExprT>(
        {make_helper<InnerExprT>(std::move(factored.reduced_first)),
...
scoretuple_t get_scoretuple(game_t match_index, team_t winning_team,
                            uint8_t reduced_points,
...
string to_string(ResultSet result_set) {
    return fmt::format("{:.3f}% ", result_set.prob * 100)
        BOOLEXPR(+to_string(result_set.which));
}
...
struct Outcomes {
   ...
   public:
    Outcomes(team_t team, scoretuple_t scores, ResultSet set)
        : team(team), result_sets{{scores, set}}, total_prob_(set.prob) {}
    Outcomes() : team(-1) {}
    ...
    void increment_total_prob(double amount) {
        ...
        if (amount + total_prob_ > 1.0000001) {
            cout << "total_prob_: " << total_prob_ << ", increment: " << amount
                 << endl;
        }
    ...
    const vector<Row> &get_rows() const {
        if (!frozen_) {
            ...
            if (fabs(total_prob() - rows_[rows_.size() - 1].cumsum_prob) >
                1e-12) {
                cout << "total_prob: " << total_prob()
                     << ", computed: " << rows_[rows_.size() - 1].cumsum_prob
                     << ", diff: "
                     << total_prob() - rows_[rows_.size() - 1].cumsum_prob
                     << "\n";
            }
            assert(fabs(total_prob() - rows_[rows_.size() - 1].cumsum_prob) <
                   1e-12);
            frozen_ = true;
        }
        return rows_;
    ...
    void update(const TeamInfo &winner, scoretuple_t this_scores,
                scoretuple_t total_scores, const ResultSet &result_set1,
                const ResultSet &result_set2, double probability) {
        assert(!frozen_);
        ResultSet new_set;
...
string to_string(const Outcomes &outcome) {
    ...
    for (const auto &scores : outcome.result_sets) {
        result += "    " + make_string(scores.first) + " " +
...
const Row &random_row(const vector<Row> &rows) {
    ...
    auto iter = lower_bound(
        rows.begin(), rows.end(), myrand,
        [](const Row &row, double myrand) { return row.cumsum_prob < myrand; });
...
vector<Outcomes> outcomes(game_t match_index, bitset<64> selections,
                          const vector<Bracket> &brackets) {
    ...
    if (ri.round == NUM_ROUNDS - 1) {
        ...
        if (game.winner >= 0) {
            ...
            double prob_first = game_prob(game.first_team, game.second_team,
                                          game.first_team, ri.round);
            teams_with_probs.push_back(
                {game.first_team,
                 {prob_first BOOLEXPR(COMMA Var::all_vars[game.id].first)}});
            teams_with_probs.push_back(
                {game.second_team,
                 {1.0 -
                  prob_first BOOLEXPR(COMMA Var::all_vars[game.id].second)}});
        }

        for (const TeamInfo &team_with_prob : teams_with_probs) {
            auto scores = get_scoretuple(match_index, team_with_prob.team,
                                         this_points / 10, brackets);
            assert(team_with_prob.team >= 0);

            if (true /* selections[team_with_prob.team] */) {
                result.emplace_back(team_with_prob.team, scores,
                                    team_with_prob.result_set);
            } else {
                result[0].result_sets[scores].prob +=
                    team_with_prob.result_set.prob;
            }
        }
    ...
    const auto outcomes1 =
        outcomes(prev_match, all_selections[match_index], brackets);
    const auto outcomes2 =
        outcomes(prev_match + 1, all_selections[match_index], brackets);

    size_t threshold_per_team_pairs =
        MONTE_CARLO_THRESHOLD / (double)(outcomes1.size() * outcomes2.size());
    ...
    */
    ...
    for (const Outcomes &outcome1 : outcomes1) {
        ...
        for (const Outcomes &outcome2 : outcomes2) {
            ...
            vector<TeamInfo> teams_with_probs;
            if (game.winner >= 0) {
                // If this game has been played in real life, then all previous
                // games have also been played, so our recursive outcomes had
                // better have only a single non-empty result.
                assert(outcomes1.size() == 1 ||
                       (outcomes1.size() == 2 &&
                        outcomes1[0].result_sets.empty()));
                assert(outcomes2.size() == 1 ||
                       (outcomes2.size() == 2 &&
                        outcomes2[0].result_sets.empty()));
                assert(game.winner == outcome1.team ||
                       game.winner == outcome2.team);
                teams_with_probs.push_back(
                    {game.winner, {1.0 BOOLEXPR(COMMA{})}});
            } else {
                double prob_first = game_prob(outcome1.team, outcome2.team,
                                              outcome1.team, ri.round);
                teams_with_probs.push_back(
                    {outcome1.team,
                     {prob_first BOOLEXPR(
                         COMMA Var::all_vars[match_index].first)}});
                teams_with_probs.push_back(
                    {outcome2.team,
                     {1.0 - prob_first BOOLEXPR(
                                COMMA Var::all_vars[match_index].second)}});
            }
            ...
            for (const auto &winner : teams_with_probs) {
                ...
                dest->increment_total_prob(winner.result_set.prob *
                                           outcome1.total_prob() *
                                           outcome2.total_prob());

                auto this_scores = get_scoretuple(match_index, winner.team,
                                                  this_points / 10, brackets);

                if (outcome1.result_sets.size() * outcome2.result_sets.size() >
                    threshold_per_team_pairs) {
                    if (!displayed_mc_warning) {
                        // cout << "Warning: Round " << round_names[ri.round] <<
                        // " using Monte Carlo simulation, results
                        // approximate.\n";
                        displayed_mc_warning = true;
                    }
                    double team_pair_prob =
                        outcome1.total_prob() * outcome2.total_prob();
                    size_t monte_carlo_iters =
                        max((size_t)(team_pair_prob * MONTE_CARLO_ITERS + 0.5),
                            (size_t)1);
                    ...
                    for (size_t i = 0; i < monte_carlo_iters; ++i) {
                        ...
                        dest->update(
                            winner, this_scores,
                            rand_row1.scoretuple + rand_row2.scoretuple,
                            rand_row1.result_set, rand_row2.result_set,
                            team_pair_prob / monte_carlo_iters);
                    }
                    // mc_iters_elapsed += elapsed(mc_iters_start, now());
                } else {
                    for (const auto &[scoretuple1, result_set1] :
                         outcome1.result_sets) {
                        for (const auto &[scoretuple2, result_set2] :
                             outcome2.result_sets) {
                            dest->update(winner, this_scores,
                                         scoretuple1 + scoretuple2, result_set1,
                                         result_set2,
    ...
    if (ri.round <= 1)
    {
       cout << "Round " << ri.round << ", elapsed " << elapsed(start, now()) <<
    " sec.\n";

       // if (rows_elapsed > 0)
       // {
       //    cout << "get_rows: " << rows_elapsed << " sec, Monte Carlo iters: "
    << mc_iters_elapsed << " sec\n";
       // }
    }
    */
...
vector<WinProb> get_win_probs(const vector<Outcomes> &outcomes) {
    ...
    for (const Outcomes &outc : outcomes) {
        for (const auto &score_and_result_sets : outc.result_sets) {
            auto [biggest_index, second_biggest_index] =
                winner(score_and_result_sets.first);
            win_probs[biggest_index].first_place.combine_disjoint(
                score_and_result_sets.second);
            win_probs[second_biggest_index].second_place.combine_disjoint(
                score_and_result_sets.second);
        }
    }
...
void alternatives(int bracket_to_consider,
                  const vector<team_t> &matches_to_consider,
                  const vector<Bracket> &brackets) {
    vector<game_t> single_eliminated;
    for (game_t match_index : matches_to_consider) {
        team_t team_must_win =
            must_win(match_index, bracket_to_consider, brackets);

        if (team_must_win > 0) {
            cout << "::::: " << teams[team_must_win].name << " (game "
                 << (int)match_index
                 << ") must win or bracket will be eliminted!\n";
    ...
    for (size_t i = 0; i < matches_to_consider.size(); i++) {
        ...
        if (find(single_eliminated.begin(), single_eliminated.end(),
                 first_match_index) != single_eliminated.end()) {
            continue;
        }
        ...
        for (size_t j = i + 1; j < matches_to_consider.size(); j++) {
            ...
            if (find(single_eliminated.begin(), single_eliminated.end(),
                     second_match_index) != single_eliminated.end()) {
                continue;
            }
            team_t team_must_win =
                must_win(second_match_index, bracket_to_consider, brackets);

            if (team_must_win > 0) {
                cout << "***** Either " << teams[matchup.second_team].name
                     << " (game " << (int)first_match_index << ") or "
                     << teams[team_must_win].name << " (game "
                     << (int)second_match_index
                     << ") must win or bracket will be eliminted!\n";
            }
        }
        ...
        for (size_t j = i + 1; j < matches_to_consider.size(); j++) {
            ...
            if (team_must_win > 0) {
                cout << "***** Either " << teams[matchup.first_team].name
                     << " (game " << (int)first_match_index << ") or "
                     << teams[team_must_win].name << " (game "
                     << (int)second_match_index
                     << ") must win or bracket will be eliminted!\n";
...
array<bool, NUM_GAMES> best_choices_2022{
...
/*
array<bool, NUM_GAMES> best_choices_2022{true, false, true, true, true, true,
...
double prob_win(const array<bool, NUM_GAMES> &choices, int entry,
                const vector<Bracket> &orig_brackets) {
...
class ProducerConsumerQueue {
   public:
    ...
    ProducerConsumerQueue(size_t num_producers, size_t num_consumers)
...
class Distributor {
   public:
    Distributor(unique_ptr<OptimGenerator> generator, int entry_to_optimize,
                const vector<Bracket> &brackets,
                int num_threads = thread::hardware_concurrency())
        : generator_(std::move(generator)),
          entry_to_optimize_(entry_to_optimize),
          brackets(brackets),
...
pair<array<bool, NUM_GAMES>, double> single_optimize(
    array<bool, NUM_GAMES> best_choices, double best_prob,
    int entry_to_optimize, game_t first_match,
    const vector<Bracket> &brackets) {
    for (game_t match = first_match; match < NUM_GAMES; ++match) {
        ...
        cout << "prob after flipping match " << (int)match << " is "
             << prob * 100 << "%\n";
        if (prob > best_prob) {
            cout << "*****  New best!\n";
...
class SingleGenerator : public OptimGenerator {
   public:
    SingleGenerator(array<bool, NUM_GAMES> best_choices)
        : best_choices_(move(best_choices)) {}
...
pair<array<bool, NUM_GAMES>, double> single_optimize_p(
    array<bool, NUM_GAMES> best_choices, double best_prob,
    int entry_to_optimize, const vector<Bracket> &brackets) {
    Distributor distrib(make_unique<SingleGenerator>(best_choices),
                        entry_to_optimize, brackets);

    return distrib.loop(best_prob);
    /*
       while (optional<Stuff> stuff = distrib.get_result())
       {
          cout << "prob after flipping match " << stuff->description << " is "
    << stuff->prob * 100 << "%\n"; if (stuff->prob > best_prob)
          {
             cout << "*****  New best!\n";
             best_choices = stuff->choices;
...
pair<array<bool, NUM_GAMES>, double> double_optimize(
    array<bool, NUM_GAMES> best_choices, double best_prob,
    int entry_to_optimize, const vector<Bracket> &brackets) {
    for (game_t outer_match = 0; outer_match < NUM_GAMES; ++outer_match) {
        ...
        double outer_prob =
            prob_win(outer_choices, entry_to_optimize, brackets);
        cout << "##### Outer.  Best prob so far " << best_prob * 100
             << "%, flipping match " << (int)outer_match
             << " gives probability " << outer_prob * 100 << "%\n";
        auto [inner_best_choices, inner_best_prob] =
            single_optimize(outer_choices, outer_prob, entry_to_optimize,
                            max(outer_match + 1, 32), brackets);
        if (inner_best_prob > best_prob) {
            best_choices = inner_best_choices;
            best_prob = inner_best_prob;
            cout << "**********  NEW OVERALL BEST!!  New best prob: "
                 << best_prob * 100 << "%\n";
        }
    }
    return make_pair(best_choices, best_prob);
...
pair<array<bool, NUM_GAMES>, double> all_optimize(
    array<bool, NUM_GAMES> initial_choices, int entry_to_optimize,
    const vector<Bracket> &brackets, const Bracket &best_ever) {
    ...
    int best_last_ever_flipped = -1;
    string best_time;

    array<bool, NUM_GAMES> best_choices;
    while (last_ever_flipped > 47) {
        array<bool, NUM_GAMES> this_choices;
        for (size_t i = 0; i < NUM_GAMES; ++i) {
            this_choices[i] =
                flipped[i] ? !initial_choices[i] : initial_choices[i];
        }
        ...
        cout << " last ever flipped " << last_ever_flipped
             << ", prob: " << prob * 100
             << "%, best flipped: " << best_last_ever_flipped << " @ "
             << best_time;
        ...
        if (prob > best_prob) {
            ...
            best_choices = this_choices;
            best_prob = prob;
            best_last_ever_flipped = last_ever_flipped;
            time_t now_time_t = system_clock::to_time_t(system_clock::now());
            best_time = ctime(&now_time_t);

            compare(make_bracket(best_choices, to_string(prob * 100) + "%"),
                    make_bracket(initial_choices, "Initial"));
            compare(make_bracket(best_choices, to_string(prob * 100) + "%"),
                    best_ever);
...
int main(int argc, char *argv[]) {
    ...
    const auto teams_json =
        get_json("espn.fantasy.maxpart.config.scoreboard_teams", html);
    ...
    const auto matchups_json =
        get_json("espn.fantasy.maxpart.config.scoreboard_matchups", html);
    ...
    for (const auto &matchup : matchups_json) {
        ...
        if (m.winner < 0) {
            ...
            const string first_name =
                (m.first_team < 0 ? to_string(m.id) + "first"
                                  : teams[m.first_team].abbrev);
            const string second_name =
                (m.second_team < 0 ? to_string(m.id) + "second"
                                   : teams[m.second_team].abbrev);
            Var::all_vars[m.id] = make_pair(
                make_shared<Var>(first_name + "-" + rname, m.id, true),
    ...
    {
       cout << "Game " << (int)i << ": " << teams[games[i].first_team].name << "
    vs " << teams[games[i].second_team].name << "\n";
    }
    cout << "Game 58 input: " << input(58) << "\n";
    cout << "   " << teams[games[input(58)].first_team].name << ", " <<
    teams[games[input(58)].second_team].name << "\n"; cout << "Game 61 input: "
    << input(61) << "\n";
    */
   ...
   vector<bool> bracket_eliminated(NUM_BRACKETS);
   {
    ...
    // Arizona going to championship, against Gonzaga.  24.24% chance of
    winning. array<bool, NUM_GAMES> single_choices{true, false, true, true,
    ...
    array<bool, NUM_GAMES> double_choices{true, false, true, true, true, true,
    ...
    false, false, false, true, // <- change in this line false, false, true,
    true, true, true};         // <- change in this line.
    */

    // 25.09%.  So only helps over choosing the most likely (21.53%) once every
    // 28 years. No single or double flip will improve it, i.e.
    // single_optimize() and double_optimze() don't change it.
    array<bool, NUM_GAMES> even_better{
    ...
    // Best result, compared to make_most_likely():
    // Sweet 16
    //   48 Gonzaga              Gonzaga
    //   49 Texas Tech           Texas Tech
    // * 50 Baylor               UCLA
    //   51 Kentucky             Kentucky
    //   52 Arizona              Arizona
    //   53 Villanova            Villanova
    // * 54 Iowa                 Kansas
    //   55 Auburn               Auburn
    // Elite 8
    //   56 Gonzaga              Gonzaga
    //   57 Kentucky             Kentucky
    // * 58 Villanova            Arizona
    // * 59 Iowa                 Kansas
    // Final 4
    //   60 Gonzaga              Gonzaga
    // * 61 Villanova            Kansas
    ...
    //   62 Gonzaga              Gonzaga
    /*
    array<bool, NUM_GAMES> single_from_most_likely{true, false, true, true,
    true, true, true, true, true, true, true, true, true, true, true, true,
    true, false, true, true, false, true, true, true, true, true, true, true,
    true, true, true, true, true, true, false, false, true, false, false, false,
    true, true, false, false, true, true, false, false, true, false, false,
    false, true, true, true, false, true, false, true, true, true, false, true};
    // compare(make_bracket(single_choices, "Single Optimized"),
    make_bracket(double_choices, "Double Optimized"));
    // compare(make_bracket(single_choices, "Single Optimized"),
    make_most_likely_bracket());
    // compare(make_bracket(double_choices, "Double Optimized"),
    make_most_likely_bracket());
    */

    /* Found during brute force optimizing, flipping round 54 (2nd last Sweet
     * 16). 24.99%
     */
    // array<bool, NUM_GAMES> brute_force{true, false, true, true, true, true,
    // true, true, true, true, true, true, true, true, true, true, true, false,
    // true, true, false, true, true, true, true, true, true, true, true, true,
    ...
    // compare(make_bracket(even_better, "Best so far"),
    // make_most_likely_bracket().first);
    ...
    auto start = now();
    auto best_ever = make_bracket(even_better, "Best Ever");
    /* auto [best_choices, best_prob] =*/all_optimize(
        to_optimize, entry_to_optimize, brackets, best_ever);

    // /* auto [best_choices, best_prob] =*/double_optimize(to_optimize, best_p,
    //                                                      entry_to_optimize,
    //                                                      brackets);
